    ...
    try:
        ...
        cursor.execute("""
            SELECT b.number AS account_number
             ...
             WHERE (d.updated_at <= %s AND d.deleted_at IS NULL)
                   OR d.id IS NULL
          GROUP BY b.number
        ...
        if update_instance.updating_type == UpdatingTypes.BY_API:
            ...
            for account_number in account_numbers:
                ...
                try:
                    ...
                    logger.info(f"Ответ ACA статус: {response.status_code}")
                    ...
                    accounts_data[account_data['accountNumber']] = [
                        {
                            ...
                            "modem": c.get('modem', ""),
                            "source": c.get('source', ''),
                            "last_check": c['previousVerificationDate'][:10] if c.get(
                                'previousVerificationDate') else None,
    ...
    try:
        for aca_account_number, aca_account_devices in aca_loaded_data.items():
            if not aca_account_number or not str(aca_account_number).strip().isdigit():
                ...
                continue
            try:
                account_id = int(aca_account_number)
            except ValueError:
                logger.error(f"Невозможно привести к int ЛС '{aca_account_number}', пропускаю")
                continue

            logger.info(f"Начинаю синхронизацию по ЛС {account_id}")
            # MAIN QUERY
            cursor.execute("""
